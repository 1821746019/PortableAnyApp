...
int path_ignored(const char* in_path) {
	//return PathJudge::_ins_()->judge(in_path);
...
#ifdef BS_DBG
	if (strstr(orig_path, "c:\\windows"))
	{
		DBG_printf("Found target");
	}
#endif
...
#if defined _WIN32
		static std::unordered_map<char*, int> path2len;
		//bool equal_or_belongTo_path = strstr(orig_path,centry->path);
		int ignore = _strnicmp(orig_path, centry->path,
			path2len.contains(centry->path) ? path2len[centry->path] : (path2len[centry->path] = strlen(centry->path))) == 0; //equal_or_belongTo_path;
		//int result = strnicmp(orig_path, centry->path, strlen(centry->path));
#else
		int judge = strncmp(orig_path, centry->path, strlen(centry->path));
#endif
		if (ignore)
		{
			/*DBG_printf("Path Ignored:%s!",orig_path);*/
			return 1;
		}

		centry = (decltype(centry))centry->next;
	}
	return 0;
}
...
void add_path_to_ignore_list(char* in_path) {
	PFS_IGNORE_ENTRY n_entry = (PFS_IGNORE_ENTRY)calloc(1, sizeof(FS_IGNORE_ENTRY));

#if defined _WIN32
	n_entry->path = (decltype(n_entry->path))malloc(strlen(in_path) + 5);
	...
	PFS_IGNORE_ENTRY centry = first_ignore_entry;
	if (centry) {
		while (centry->next) { centry = (PFS_IGNORE_ENTRY)centry->next; }
	}
	centry->next = n_entry;
}

void init_fs_ignore_list() {
	char* ignore_list = getenv("PDXFS_IGNORE");
	if (!ignore_list) { return; }
	const char* envbl = ignore_list;
	char* envbl_tmp = (char*)calloc(1, strlen(envbl) + 1);
...
void generate_path_info(PPDXPATH sp) {
	...
	sp->redirected_path = (char*)calloc(1, 1024);
	...
	sp->create_parent_path = 1; (sp->original_parent_exists && !sp->redirected_parent_exists);
	}
...
static bool enable_fs_redirect = false;
void enableFS_Redirect()
{
	enable_fs_redirect = true;
}
void disableFS_Redirect()
{
	enable_fs_redirect = false;
}
int fs_redirect(char* in_abspath, int is_directory, int is_read, int is_write, int fail_if_exist, int fail_if_not_exist, char** redirected_path) {
	if (!enable_fs_redirect)return 0;
	if (!fs_init) {
		init_fs();
	...
	if (!fs_root) { return 0; }
	// Check judge list and return at this point.    
	if (path_ignored(in_abspath)) { return 0; }

	PPDXPATH sp = (PPDXPATH)calloc(1, sizeof(PDXPATH));
