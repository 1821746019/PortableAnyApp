...
tNtCreateFile* ntdll_NtCreateFile = NULL;
tNtOpenFile* ntdll_NtOpenFile = NULL;
tNtOpenDirectoryObject* ntdll_NtOpenDirectoryObject = NULL;
tNtCreateDirectoryObject* ntdll_NtCreateDirectoryObject = NULL;
tNtCreateDirectoryObjectEx* ntdll_NtCreateDirectoryObjectEx = NULL;
tNtQueryAttributesFile* ntdll_NtQueryAttributesFile = NULL;
tNtQueryFullAttributesFile* ntdll_NtQueryFullAttributesFile = NULL;
tLdrLoadDll* ntdll_LdrLoadDll = NULL;
...
NTSTATUS wrapped_createdir(wchar_t* path) {
	...
	NTSTATUS status = ntdll_NtCreateFile(&hObject, desired_access, &pathOa, &IoSb, NULL, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_CREATE, FILE_DIRECTORY_FILE | FILE_OPEN_FOR_BACKUP_INTENT | FILE_OPEN_REPARSE_POINT | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	if (hObject) {
		NtClose(hObject);
	}
...
NTSTATUS wrapped_createfile(wchar_t* path, PHANDLE hObject, DWORD add_create_options) {
	...
	desired_access |= FLAG_BYPASS;
#endif
	return ntdll_NtCreateFile(hObject, desired_access, &pathOa, &IoSb, NULL, 0, 0, FILE_OPEN, create_options, NULL, 0);
}
...
void delete_file_native(wchar_t* path) {
	...
	desired_access |= FLAG_BYPASS;
#endif    
	NTSTATUS status = ntdll_NtOpenFile(&hObject, desired_access, &pathOa, &IoSb, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_NON_DIRECTORY_FILE | FILE_OPEN_FOR_BACKUP_INTENT | FILE_OPEN_REPARSE_POINT);
	if (status) { return; }
...
void copy_file_native(wchar_t* src_path, wchar_t* dst_path) {
	...
	NTSTATUS status = ntdll_NtCreateFile(&hSrc, desired_access_src, &pathOa, &IoSb, NULL, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
	if (status) { return; }
	...
	while (offset < fsi.EndOfFile.QuadPart) {
		if ((fsi.EndOfFile.QuadPart - offset) < chunk_size) {
			chunk_size = (unsigned)(fsi.EndOfFile.QuadPart - offset);
		}
...
BOOL get_device_root_string(wchar_t* in_path, wchar_t* out_path, unsigned int* tail_offset) {
	...
	*tail_offset = (unsigned)device_root_size + 1;
