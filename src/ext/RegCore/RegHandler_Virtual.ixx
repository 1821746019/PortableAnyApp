module;
#include <Windows.h>
export module RegHandler_Virtual;
import std;
import RegHandler_B;
import func2hook.kernel.raw;
import AppRegHive;

export class RegHandler_Virtual : public RegHandler_B {
 public:
  LSTATUS 
  ...
  LSTATUS  RegQueryInfoKeyW(HKEY hKey,
                                  LPWSTR lpClass,
                                  LPDWORD lpcClass,
                                  LPDWORD lpReserved,
                                  LPDWORD lpcSubKeys,
  ...
  LSTATUS  RegEnumKeyExW(HKEY hKey,
                               DWORD dwIndex,
                               LPWSTR lpName,
                               LPDWORD lpcName,
                               LPDWORD lpReserved,
  ...
  LSTATUS RegEnumValueW(HKEY hkey,
      DWORD dwIndex,
      LPWSTR lpValueName,
      ...
      LPBYTE lpData,
                        LPDWORD lpcbData) override {
    return RegEnumValueW_raw(hkey, dwIndex, lpValueName, lpcchValueName,
                             lpReserved, lpType, lpData, lpcbData);
  }
  LSTATUS RegSetValueExW(HKEY hKey,
                         LPCWSTR lpValueName,
                         DWORD Reserved,
                         DWORD dwType,
                         const BYTE* lpData,
                         DWORD cbData) override {
    return RegSetValueExW_raw(hKey, lpValueName, Reserved, dwType, lpData,
                              cbData);
  }
};