...
export{
	...
	public:
		...
		MacroMgr(const std::unordered_map<std::wstring, std::wstring>& macro_map) :macro_map_(macro_map)
		{
		}
		...
		void add(const std::unordered_map<std::wstring, std::wstring>& macro_map)
		{
			...
			{
				//do not allow to overwrite the builtin macro or assign more than one time
				if(macro_map_.contains(k))continue;
				macro_map_.emplace(k, v);

			}
		}
		void remove(const std::wstring& macro)
		{
			macro_map_.erase(macro);
		}
