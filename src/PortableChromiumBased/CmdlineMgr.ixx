...
size_t getEndIdx(const string& cmdline, size_t offset, bool already_in_quotes = false)
{
...
size_t getPathFromCmdline(const string& cmdline, const string& path_flag, string* out)
{
...
int procCmdlinePathFlag(string& cmdline, const string& path_flag,  string path)
{
	...
	else//find the flag, replace it
	{
		...
		}else //not found, replace it
		{
			...
			if (cmdline[idx - 1] == '\"')need_quote = false;
			if (need_quote)
			{
				quotePathIfNeed(&path);
			...
			if (cmdline.find(cmdline_old) != -1)ret = false;
		}
...
export{
	bool procCmdline(string cmdline, string* out)
	{
		...
		bool status2 = false; //procCmdlinePathFlag(cmdline, cache_flag, cache_path);
		if (out)*out = std::move(cmdline);
