...
//UNICODE_STRING效果同上
#define procRedirect() 	std::wstring new_path_buffer;\
UNICODE_STRING new_path;\
auto is_redirected=judgeAndRedirect(ObjectAttributes, new_path_buffer, new_path, __FUNCTION__)


//bool judgeAndRedirect(POBJECT_ATTRIBUTES old_PA, UNICODE_STRING* _new_path, const char* func_name = nullptr)
//{
//	wchar_t buffer[MAX_PATH];
//	bool is_redirected = path_judge->judgeAndRedirect(old_PA->ObjectName->Buffer, buffer, (int)std::size(buffer));
...
bool doRedirect(const POBJECT_ATTRIBUTES OA, POBJECT_ATTRIBUTES _objAtrr_new, UNICODE_STRING* _filePath, wchar_t* _buffer, int len)
{
	PathJudge* path_judge = PathJudge::_ins_();
	if (!path_judge)return false;
...
#define redirectIt OBJECT_ATTRIBUTES objAtrr_new;\
UNICODE_STRING filePath;\
wchar_t buffer[MAX_PATH + 1];\
if (doRedirect(ObjectAttributes, &objAtrr_new, &filePath, buffer, (int)std::size(buffer)))\
{\
...
export
{
	...
	auto NTAPI NtCreateFile_mod(PHANDLE FileHandle,
		ACCESS_MASK DesiredAccess, POBJECT_ATTRIBUTES ObjectAttributes, PIO_STATUS_BLOCK IoStatusBlock,
		...
		ULONG CreateOptions, PVOID EaBuffer, ULONG EaLength)
	{
		...
		auto ret = NtCreateFile_raw(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock,
		...
		PIO_STATUS_BLOCK IoStatusBlock, ULONG ShareAccess, ULONG OpenOptions)
	{
		...
		NTSTATUS ret = NtOpenFile_raw(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock,
	...
	decltype(&NtQueryAttributesFile) NtQueryAttributesFile_raw = NtQueryAttributesFile;
	...
	decltype(&NtQueryFullAttributesFile) NtQueryFullAttributesFile_raw = NtQueryFullAttributesFile;
	...
	decltype(&NtSetInformationFile) NtSetInformationFile_raw = NtSetInformationFile;
	...
	NTSTATUS NTAPI NtSetInformationFile_mod(HANDLE FileHandle, PIO_STATUS_BLOCK IoStatusBlock, PVOID FileInformation, ULONG Length, FILE_INFORMATION_CLASS FileInformationClass
	)
	{
		...
		if (FileInformationClass == FileRenameInformation)
		{
			...
			p[FRI->FileNameLength] = 0; p[FRI->FileNameLength + 1] = 0;
			if (PathJudge::_ins_()->judgeAndRedirect(path_old, path_new, (int)std::size(path_new)))
			{
				auto path_len = wcslen(path_new);
				uint32_t path_byte_num = uint32_t(path_len +1) * sizeof(wchar_t);

				FRI_new = (FILE_RENAME_INFO*) new uint8_t[sizeof(FILE_RENAME_INFO) + path_byte_num];
				*FRI_new = *FRI;
		...
		else if (FileInformationClass == FileLinkInformation)
		{
			...
			uint8_t* p = (uint8_t*)path_old;
			p[FLI->FileNameLength] = 0; p[FLI->FileNameLength + 1] = 0;
			if (PathJudge::_ins_()->judgeAndRedirect(path_old, path_new, (int)std::size(path_new)))
			{
				auto path_len = wcslen(path_new);
				uint32_t path_byte_num = uint32_t(path_len+1) * sizeof(wchar_t);

				FLI_new = (FILE_LINK_INFORMATION*) new uint8_t[sizeof(FILE_RENAME_INFO) + path_byte_num];
				*FLI_new = *FLI;
		...
		if (FLI_new)FileInformation = FLI_new;
		auto ret = NtSetInformationFile_raw(FileHandle, IoStatusBlock, FileInformation, Length,
